define(["dojo/_base/declare",
    ...
    var Editor = declare(Presenter, {
        ...
        addText: function (fieldDiv, binding, noCardinalityButtons) {
            ...
            //If certain datatype
            if (nodeType == "DATATYPE_LITERAL" || datatype || nodeType=="URI") {
				
				// If URI or anyURI datatype must start with schema Stanislav Beran
                if (nodeType === "URI" || datatype === "http://www.w3.org/2001/XMLSchema#anyURI") {
				console.log("anyURI");
                    tb = new ValidationTextBox({
                        value: binding.getValue(),
                        disabled: !item.isEnabled(),
                        invalidMessage: "Make sure link starts with schema (i.e http://) and is valid url",
                        regExp: "(http|https|ftp|smtp)://([^\\s][\\w/\.]+)",
                        onChange: function () {
                            if (tb.isValid()) {
